...
func NewQueryRepository(db *gorm.DB) user.QueryRepository {
	return &queryRepository{db: db}
}

// FindByUserAndKey 根据用户 ID 和键名查找用户配置
// 如果不存在返回 nil, nil
func (r *queryRepository) FindByUserAndKey(ctx context.Context, userID uint, key string) (*user.UserSetting, error) {
...
func (r *queryRepository) FindByUser(ctx context.Context, userID uint) ([]*user.UserSetting, error) {
...
func (r *queryRepository) FindByUserAndKeys(ctx context.Context, userID uint, keys []string) ([]*user.UserSetting, error) {
	if len(keys) == 0 {
		return []*user.UserSetting{}, nil
	}

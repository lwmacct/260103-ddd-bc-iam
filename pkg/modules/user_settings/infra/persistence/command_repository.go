...
func NewCommandRepository(db *gorm.DB) userset.CommandRepository {
	return &commandRepository{db: db}
}

// Upsert 创建或更新用户配置（基于 user_id + setting_key 唯一约束）
func (r *commandRepository) Upsert(ctx context.Context, setting *userset.UserSetting) error {
...
func (r *commandRepository) BatchUpsert(ctx context.Context, settings []*userset.UserSetting) error {
	if len(settings) == 0 {
		return nil
	}

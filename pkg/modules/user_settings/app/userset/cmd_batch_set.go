...
func (h *BatchSetHandler) Handle(ctx context.Context, cmd BatchSetCommand) ([]*UserSettingDTO, error) {
	...
	// 3. 校验并构建用户配置列表
	userSettings := make([]*userset.UserSetting, 0, len(cmd.Settings))
	for _, item := range cmd.Settings {
		def, ok := defMap[item.Key]
		if !ok {
			return nil, fmt.Errorf("%w: %s", userset.ErrInvalidSettingKey, item.Key)
		}

		// 检查是否为用户可配置的配置
		if !def.IsUserScope() {
			return nil, fmt.Errorf("%w: %s is not a user-configurable setting", userset.ErrInvalidSettingKey, item.Key)
		}

		// 类型校验
		if err := def.ValidateValue(item.Value); err != nil {
			return nil, fmt.Errorf("%w: %s - %w", userset.ErrInvalidSettingValue, item.Key, err)
		}

		// 格式校验
		if err := def.ValidateByInputType(item.Value); err != nil {
			return nil, fmt.Errorf("%w: %s - %w", userset.ErrValidationFailed, item.Key, err)
		}

		userSettings = append(userSettings, userset.New(cmd.UserID, item.Key, item.Value))
	}

...
import (
	...
	usersetDomain "github.com/lwmacct/260103-ddd-bc-iam/pkg/modules/user_settings/domain/userset"
)

// UserSettingHandler 用户配置 HTTP Handler
type UserSettingHandler struct {
	setHandler      *userset.SetHandler
	batchSetHandler *userset.BatchSetHandler
	resetHandler    *userset.ResetHandler
	resetAllHandler *userset.ResetAllHandler
	getHandler      *userset.GetHandler
	listHandler     *userset.ListHandler
	listCatHandler  *userset.ListCategoriesHandler
}
...
//	@Success		200		{object}	response.DataResponse[[]userset.UserSettingDTO]	"配置列表"
...
//	@Produce		json
//	@Security		BearerAuth
//	@Success		200		{object}	response.DataResponse[[]userset.CategoryDTO]	"分类列表"
//	@Failure		401		{object}	response.ErrorResponse	"未授权"
//	@Failure		500		{object}	response.ErrorResponse	"服务器内部错误"
...
func (h *UserSettingHandler) ListCategories(c *gin.Context) {
	...
	query := userset.ListCategoriesQuery{
		UserID: uid,
	}
...
//	@Success		200		{object}	response.DataResponse[userset.UserSettingDTO]	"配置信息"
...
func (h *UserSettingHandler) Get(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, usersetDomain.ErrInvalidSettingKey) {
...
//	@Success		200		{object}	response.DataResponse[userset.UserSettingDTO]	"设置成功"
...
func (h *UserSettingHandler) Set(c *gin.Context) {
	...
	if err != nil {
		...
		if errors.Is(err, usersetDomain.ErrInvalidSettingValue) {
			response.BadRequest(c, "invalid setting value", err.Error())
			return
		}
		if errors.Is(err, usersetDomain.ErrValidationFailed) {
...
func (h *UserSettingHandler) BatchSet(c *gin.Context) {
	...
	// 转换请求
	items := make([]userset.SettingItem, len(req.Settings))
	for i, s := range req.Settings {
		items[i] = userset.SettingItem{
			Key:   s.Key,
			Value: s.Value,
		}
	}

	cmd := userset.BatchSetCommand{
		UserID:   uid,
		Settings: items,
	}

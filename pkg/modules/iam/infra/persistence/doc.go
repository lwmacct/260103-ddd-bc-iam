// Package persistence 提供 IAM 模块的数据库持久化实现。
//
// # Overview
//
// 本包实现 Domain 层定义的所有 Repository 接口，提供完整的 CRUD 操作：
//   - User Repositories: 用户仓储（含缓存装饰器）
//   - Role Repositories: 角色仓储
//   - PAT Repositories: 个人访问令牌仓储
//   - TwoFA Repositories: 双因素认证仓储
//   - Audit Repositories: 审计日志仓储
//   - Org Repositories: 组织、组织成员、团队、团队成员仓储
//
// # CQRS 模式
//
// 每个领域模块都有独立的 Command 和 Query 仓储：
//   - Command Repository: 写操作（Create、Update、Delete）
//   - Query Repository: 读操作（Get、List、Exists、Search）

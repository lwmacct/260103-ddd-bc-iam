...
func newPermissionCacheService(client *redis.Client, iamCfg *iamconfig.Config) appauth.PermissionCacheService {
	return NewPermissionCacheService(client, iamCfg.RedisCache.KeyPrefix)
}

// newUserWithRolesCacheService 创建用户实体缓存服务。
func newUserWithRolesCacheService(client *redis.Client, iamCfg *iamconfig.Config) appuser.UserWithRolesCacheService {

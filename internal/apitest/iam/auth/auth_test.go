...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestLoginScenarios ./internal/integration/auth/
func TestLoginScenarios(t *testing.T) {
	iam.SkipIfNotAPITest(t)
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestGetCaptcha ./internal/integration/auth/
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestAuthFlow ./internal/integration/auth/
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestRegister ./internal/integration/auth/
func TestRegister(t *testing.T) {
	...
	resp, err := apitest.Post[auth.RegisterResultDTO](c.HTTPClient(), "/api/auth/register", registerReq)
	...
	adminClient := iam.NewClientFromConfig()
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestRegisterDuplicate ./internal/integration/auth/
func TestRegisterDuplicate(t *testing.T) {
	...
	firstResp, err := apitest.Post[auth.RegisterResultDTO](c.HTTPClient(), "/api/auth/register", registerReq)
	...
	_, err = apitest.Post[auth.RegisterResultDTO](c.HTTPClient(), "/api/auth/register", duplicateReq)
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestRefreshTokenScenarios ./internal/integration/auth/
func TestRefreshTokenScenarios(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			resp, err := apitest.Post[auth.RefreshTokenResultDTO](c.HTTPClient(), "/api/auth/refresh", refreshReq)
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestGetCurrentUser ./internal/integration/auth/
func TestGetCurrentUser(t *testing.T) {
	...
	t.Log("步骤 2: 获取当前用户信息")
	me, err := apitest.Get[user.UserWithRolesDTO](c.HTTPClient(), "/api/user/profile", nil)

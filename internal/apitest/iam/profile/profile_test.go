...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestGetProfile ./internal/integration/profile/
func TestGetProfile(t *testing.T) {
	c := iam.LoginAsAdmin(t)

	t.Log("获取个人资料")
	profile, err := apitest.Get[user.UserWithRolesDTO](c.HTTPClient(), "/api/user/profile", nil)
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestUpdateProfile ./internal/integration/profile/
func TestUpdateProfile(t *testing.T) {
	c := iam.LoginAsAdmin(t)

	t.Log("步骤 1: 获取当前资料")
	originalProfile, err := apitest.Get[user.UserWithRolesDTO](c.HTTPClient(), "/api/user/profile", nil)
	...
	t.Cleanup(func() {
		...
		_, _ = apitest.Put[user.UserWithRolesDTO](c.HTTPClient(), "/api/user/profile", restoreReq)
	})
	...
	updateResp, err := apitest.Put[user.UserWithRolesDTO](c.HTTPClient(), "/api/user/profile", updateReq)
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestUpdateProfileInvalid ./internal/integration/profile/
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestChangePassword ./internal/integration/profile/
func TestChangePassword(t *testing.T) {
	...
	// 用测试用户登录
	t.Log("步骤 2: 测试用户登录")
	testClient := iam.LoginAs(t, testUsername, originalPassword)
	t.Log("  登录成功")
	...
	t.Log("步骤 4: 使用新密码登录")
	newClient := iam.LoginAs(t, testUsername, newPassword)
	_ = newClient
	t.Log("  新密码登录成功!")

	t.Log("步骤 5: 验证旧密码已失效")
	oldPwdClient := iam.NewClientFromConfig()
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestChangePasswordWrongOld ./internal/integration/profile/
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestDeleteAccount ./internal/integration/profile/
func TestDeleteAccount(t *testing.T) {
	...
	verifyClient := iam.NewClientFromConfig()

...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestAdminUsersFlow ./internal/integration/user/
func TestAdminUsersFlow(t *testing.T) {
	c := iam.LoginAsAdmin(t)

	// 测试 1: 获取用户列表
	t.Log("\n测试 1: 获取用户列表")
	users, meta, err := apitest.GetList[user.UserDTO](c.HTTPClient(), "/api/admin/users", map[string]string{})
	...
	t.Log("\n测试 2: 创建用户")
	testUser, markDeleted := iam.CreateTestUserWithCleanupControl(t, c, "testuser")
	...
	// 测试 3: 获取用户详情
	t.Log("\n测试 3: 获取用户详情")
	userDetail, err := apitest.Get[user.UserDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/users/%d", testUser.ID), nil)
	...
	updatedUser, err := apitest.Put[user.UserDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/users/%d", testUser.ID), updateReq)
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestListUsers ./internal/integration/user/
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestAssignRoles ./internal/integration/user/
func TestAssignRoles(t *testing.T) {
	c := iam.LoginAsAdmin(t)

	// 创建测试用户
	t.Log("\n步骤 1: 创建测试用户")
	testUser := iam.CreateTestUser(t, c, "roletest")
	t.Logf("  创建成功! 用户 ID: %d", testUser.ID)
	...
	assignResp, err := apitest.Put[user.UserWithRolesDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/users/%d/roles", testUser.ID), assignReq)
	...
	roleIDs := apitest.ExtractIDs(assignResp.Roles, func(r user.RoleDTO) uint { return r.ID })
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestBatchCreateUsers ./internal/integration/user/
func TestBatchCreateUsers(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		for _, username := range []string{username1, username2} {
			...
			users, _, _ := apitest.GetList[user.UserWithRolesDTO](c.HTTPClient(), "/api/admin/users", searchParams)
			for _, u := range users {
	...
	result, err := apitest.Post[user.BatchCreateResultDTO](c.HTTPClient(), "/api/admin/users/batch", batchReq)
	...
	users1, _, _ := apitest.GetList[user.UserWithRolesDTO](c.HTTPClient(), "/api/admin/users", searchParams1)
	...
	users2, _, _ := apitest.GetList[user.UserWithRolesDTO](c.HTTPClient(), "/api/admin/users", searchParams2)
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestSystemUserProtection ./internal/integration/user/
func TestSystemUserProtection(t *testing.T) {
	c := iam.LoginAsAdmin(t)

	// 获取系统用户
	t.Log("\n步骤 1: 获取系统用户")
	users, _, err := apitest.GetList[user.UserDTO](c.HTTPClient(), "/api/admin/users", nil)
	require.NoError(t, err, "获取用户列表失败")
	...
	_, err = apitest.Put[user.UserDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/users/%d", rootUser.ID), updateReq)
	...
	_, err = apitest.Put[user.UserDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/users/%d", rootUser.ID), statusReq)
	...
	updatedAdmin, err := apitest.Put[user.UserDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/users/%d", adminUser.ID), emailReq)
	...
	_, _ = apitest.Put[user.UserDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/users/%d", adminUser.ID), restoreReq)
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestRootRoleProtection ./internal/integration/user/
func TestRootRoleProtection(t *testing.T) {
	...
	_, err = apitest.Put[user.UserWithRolesDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/users/%d/roles", rootUser.ID), assignReq)

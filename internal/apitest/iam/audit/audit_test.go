...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestListAuditLogs ./internal/integration/audit/
func TestListAuditLogs(t *testing.T) {
	c := iam.LoginAsAdmin(t)

	t.Log("\n获取审计日志列表...")
	logs, meta, err := apitest.GetList[audit.AuditDTO](c.HTTPClient(), "/api/admin/audit", map[string]string{})
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestGetAuditLogDetail ./internal/integration/audit/
func TestGetAuditLogDetail(t *testing.T) {
	c := iam.LoginAsAdmin(t)

	// 先获取日志列表，取第一条的 ID
	t.Log("\n步骤 1: 获取日志列表")
	logs, _, err := apitest.GetList[audit.AuditDTO](c.HTTPClient(), "/api/admin/audit", map[string]string{})
	require.NoError(t, err, "获取审计日志列表失败")
	...
	// 获取详情
	t.Log("\n步骤 2: 获取日志详情")
	detail, err := apitest.Get[audit.AuditDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/audit/%d", logID), nil)
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestAuditLogFilters ./internal/integration/audit/
func TestAuditLogFilters(t *testing.T) {
	c := iam.LoginAsAdmin(t)

	// 测试按操作类型筛选
	t.Log("\n测试 1: 按操作类型筛选 (login)")
	loginLogs, meta, err := apitest.GetList[audit.AuditDTO](c.HTTPClient(), "/api/admin/audit", map[string]string{})
	if err != nil {
	...
	userLogs, meta, err := apitest.GetList[audit.AuditDTO](c.HTTPClient(), "/api/admin/audit", map[string]string{})
	if err != nil {
		t.Logf("  筛选失败: %v（可能不支持此筛选）", err)
	} else {
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestAuditLogNotFound ./internal/integration/audit/
func TestAuditLogNotFound(t *testing.T) {
	...
	_, err := apitest.Get[audit.AuditDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/audit/%d", invalidID), nil)

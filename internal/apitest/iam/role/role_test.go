...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestRolesFlow ./internal/integration/role/
func TestRolesFlow(t *testing.T) {
	c := iam.LoginAsAdmin(t)

	// 测试 1: 获取角色列表
	t.Log("\n测试 1: 获取角色列表")
	roles, meta, err := apitest.GetList[role.RoleDTO](c.HTTPClient(), "/api/admin/roles", map[string]string{})
	...
	// 测试 2: 创建角色（使用工厂函数）
	t.Log("\n测试 2: 创建角色")
	testRole, markDeleted := iam.CreateTestRoleWithCleanupControl(t, c, "testrole")
	...
	// 测试 3: 获取角色详情
	t.Log("\n测试 3: 获取角色详情")
	roleDetail, err := apitest.Get[role.RoleDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/roles/%d", testRole.RoleID), nil)
	...
	updatedRole, err := apitest.Put[role.RoleDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/roles/%d", testRole.RoleID), updateReq)
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestListRoles ./internal/integration/role/
...
func testSetRolePermissions(t *testing.T, c *iam.Client, roleID uint) {
	t.Helper()
	...
	// 验证权限已设置
	roleWithPerms, err := apitest.Get[role.RoleDTO](c.HTTPClient(), fmt.Sprintf("/api/admin/roles/%d", roleID), nil)
	require.NoError(t, err, "获取角色详情失败")
...
// 手动运行:
//
//	API_TEST=1 go test -v -run TestSystemRoleProtection ./internal/integration/role/
func TestSystemRoleProtection(t *testing.T) {
	c := iam.LoginAsAdmin(t)

	// 获取系统角色
	t.Log("\n步骤 1: 获取系统角色")
	roles, _, err := apitest.GetList[role.RoleDTO](c.HTTPClient(), "/api/admin/roles", nil)
	require.NoError(t, err, "获取角色列表失败")

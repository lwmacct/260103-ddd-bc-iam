...
func TestListOrgSettings(t *testing.T) {
	...
	t.Run("获取组织设置列表应成功", func(t *testing.T) {
		result, _, err := apitest.GetList[org.SettingsItemDTO](c.HTTPClient(),
...
func TestGetOrgSetting(t *testing.T) {
	...
	t.Run("获取存在的设置", func(t *testing.T) {
		...
		result, err := apitest.Get[org.SettingsItemDTO](c.HTTPClient(),
	...
	t.Run("获取不存在的设置应返回错误", func(t *testing.T) {
		_, err := apitest.Get[org.SettingsItemDTO](c.HTTPClient(),
...
func TestSetOrgSetting(t *testing.T) {
	...
	t.Run("设置组织自定义值", func(t *testing.T) {
		...
		result, err := apitest.Put[org.SettingsItemDTO](c.HTTPClient(),
	...
	t.Run("验证自定义值覆盖系统默认值", func(t *testing.T) {
		// 先获取当前值
		original, err := apitest.Get[org.SettingsItemDTO](c.HTTPClient(),
		...
		_, err = apitest.Put[org.SettingsItemDTO](c.HTTPClient(),
			orgSettingPath(testOrgID, "general.language"),
			updateReq,
		)
		...
		// 验证自定义值
		updated, err := apitest.Get[org.SettingsItemDTO](c.HTTPClient(),
			orgSettingPath(testOrgID, "general.language"),
			nil,
		)
...
func TestResetOrgSetting(t *testing.T) {
	...
	t.Run("重置组织自定义值", func(t *testing.T) {
		...
		// 验证自定义值存在
		customResult, err := apitest.Get[org.SettingsItemDTO](c.HTTPClient(),
			orgSettingPath(testOrgID, testKey),
			nil,
		)
		...
		// 验证恢复为系统默认值
		defaultResult, err := apitest.Get[org.SettingsItemDTO](c.HTTPClient(),
			orgSettingPath(testOrgID, testKey),
			nil,
		)
...
func TestOrgSettingsPermission(t *testing.T) {
	...
	t.Run("非组织成员访问组织设置应被拒绝", func(t *testing.T) {
		...
		// 以测试用户登录
		nonMemberClient := settings.LoginAs(t, "test_non_org_member", "Test123456!")

		// 尝试访问组织设置（应被拒绝）
		_, _, err = apitest.GetList[org.SettingsItemDTO](nonMemberClient.HTTPClient(),
			orgSettingsPath(testOrgID),
			nil,
		)
	...
	t.Run("组织成员可以访问组织设置", func(t *testing.T) {
		...
		result, _, err := apitest.GetList[org.SettingsItemDTO](adminClient.HTTPClient(),
			orgSettingsPath(testOrgID),
			nil,
		)
	...
	t.Run("组织普通成员无法访问组织设置", func(t *testing.T) {
		...
		// 以测试用户登录
		memberClient := settings.LoginAs(t, "test_org_member", "Test123456!")

		// member 角色无法读取组织设置（需要 owner/admin 角色）
		_, _, err = apitest.GetList[org.SettingsItemDTO](memberClient.HTTPClient(),
			orgSettingsPath(testOrgID),
			nil,
		)
	...
	t.Run("组织管理员可以访问组织设置", func(t *testing.T) {
		...
		// 以测试用户登录
		orgAdminClient := settings.LoginAs(t, "test_org_admin", "Test123456!")

		// org admin 角色可以读取组织设置（通过 OrgContext 动态权限注入）
		result, _, err := apitest.GetList[org.SettingsItemDTO](orgAdminClient.HTTPClient(),
			orgSettingsPath(testOrgID),
			nil,
		)
		...
		_, err = apitest.Put[org.SettingsItemDTO](orgAdminClient.HTTPClient(),
			orgSettingPath(testOrgID, "general.theme"),
			updateReq,
		)
	...
	t.Run("组织 Owner 可以读取和修改设置", func(t *testing.T) {
		...
		_, err = apitest.Put[org.SettingsItemDTO](adminClient.HTTPClient(),
			orgSettingPath(testOrgID, "general.theme"),
			updateReq,
		)
...
func TestOrgSettingsValueTypes(t *testing.T) {
	...
	t.Run("字符串类型设置", func(t *testing.T) {
		...
		getResult, err := apitest.Get[org.SettingsItemDTO](c.HTTPClient(),
			orgSettingPath(testOrgID, "general.theme"),
			nil,
		)
		require.NoError(t, err, "获取设置应成功")

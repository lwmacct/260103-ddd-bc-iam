...
func TestListSettings(t *testing.T) {
	...
	t.Run("登录用户可获取设置列表", func(t *testing.T) {
		result, _, err := apitest.GetList[user.SettingsItemDTO](c.HTTPClient(),
			"/api/user/settings",
			nil,
		)
...
func TestGetSetting(t *testing.T) {
	...
	t.Run("获取存在的设置", func(t *testing.T) {
		...
		result, err := apitest.Get[user.SettingsItemDTO](c.HTTPClient(),
...
func TestUpdateSetting(t *testing.T) {
	...
	t.Run("更新设置创建自定义值", func(t *testing.T) {
		...
		result, err := apitest.Put[user.SettingsItemDTO](c.HTTPClient(),
		...
		// 验证更新后的值
		getResult, err := apitest.Get[user.SettingsItemDTO](c.HTTPClient(),
			"/api/user/settings/"+testKey,
			nil,
		)
	...
	t.Run("验证自定义值覆盖系统默认值", func(t *testing.T) {
		// 先获取原始值
		original, err := apitest.Get[user.SettingsItemDTO](c.HTTPClient(),
		...
		_, err = apitest.Put[user.SettingsItemDTO](c.HTTPClient(),
			"/api/user/settings/general.language",
			updateReq,
		)
		...
		// 验证自定义值
		updated, err := apitest.Get[user.SettingsItemDTO](c.HTTPClient(),
			"/api/user/settings/general.language",
			nil,
		)
...
func TestDeleteSetting(t *testing.T) {
	...
	t.Run("删除自定义值恢复系统默认", func(t *testing.T) {
		...
		// 验证自定义值存在
		customResult, err := apitest.Get[user.SettingsItemDTO](c.HTTPClient(),
			"/api/user/settings/"+testKey,
			nil,
		)
		...
		// 验证恢复为系统默认值
		defaultResult, err := apitest.Get[user.SettingsItemDTO](c.HTTPClient(),
			"/api/user/settings/"+testKey,
			nil,
		)
...
func TestListCategories(t *testing.T) {
	...
	t.Run("获取分类列表应成功", func(t *testing.T) {
		result, _, err := apitest.GetList[user.CategoryDTO](c.HTTPClient(),
...
func TestBatchSet(t *testing.T) {
	...
	t.Run("批量设置多个配置项", func(t *testing.T) {
		...
		_, err := apitest.Post[any](c.HTTPClient(), "/api/user/settings/batch",
			batchReq,
		)
		require.NoError(t, err, "批量设置应成功")

		// 验证 theme 设置
		themeResult, err := apitest.Get[user.SettingsItemDTO](c.HTTPClient(),
			"/api/user/settings/general.theme",
			nil,
		)
		...
		// 验证 language 设置
		langResult, err := apitest.Get[user.SettingsItemDTO](c.HTTPClient(),
			"/api/user/settings/general.language",
			nil,
		)
...
func TestResetAll(t *testing.T) {
	...
	t.Run("重置所有配置", func(t *testing.T) {
		...
		// 验证自定义值存在
		themeBeforeReset, err := apitest.Get[user.SettingsItemDTO](c.HTTPClient(),
			"/api/user/settings/general.theme",
			nil,
		)
		require.NoError(t, err, "获取设置应成功")
		...
		// 执行重置所有
		_, err = apitest.Post[any](c.HTTPClient(), "/api/user/settings/reset-all",
			nil,
		)
		require.NoError(t, err, "重置所有配置应成功")

		// 验证所有配置已恢复默认值
		themeAfterReset, err := apitest.Get[user.SettingsItemDTO](c.HTTPClient(),
			"/api/user/settings/general.theme",
			nil,
		)
		require.NoError(t, err, "获取设置应成功")
		...
		langAfterReset, err := apitest.Get[user.SettingsItemDTO](c.HTTPClient(),
			"/api/user/settings/general.language",
			nil,
		)
		require.NoError(t, err, "获取设置应成功")
...
func TestSettingsIsolation(t *testing.T) {
	...
	// 以测试用户登录
	testUserClient := settings.LoginAs(t, "test_user_settings", "Test123456!")
	t.Cleanup(func() {
	...
	t.Run("不同用户的自定义值应独立", func(t *testing.T) {
		...
		_, err = apitest.Put[user.SettingsItemDTO](adminClient.HTTPClient(),
			"/api/user/settings/"+testKey,
			adminUpdateReq,
		)
		...
		_, err = apitest.Put[user.SettingsItemDTO](testUserClient.HTTPClient(),
			"/api/user/settings/"+testKey,
			testUserUpdateReq,
		)
		...
		// 验证 Admin 的值
		adminResult, err := apitest.Get[user.SettingsItemDTO](adminClient.HTTPClient(),
			"/api/user/settings/"+testKey,
			nil,
		)
		...
		// 验证测试用户的值
		testUserResult, err := apitest.Get[user.SettingsItemDTO](testUserClient.HTTPClient(),
			"/api/user/settings/"+testKey,
			nil,
		)

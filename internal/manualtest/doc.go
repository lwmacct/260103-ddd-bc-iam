...
//	Go 1.10+ 默认缓存测试结果，manualtest 依赖外部服务状态，
//	禁用缓存确保每次测试都真正执行，避免假阴性。
//
//	# 运行所有测试
//	MANUAL=1 go test -v -count=1 ./internal/manualtest/... 2>&1 | grep -E "FAIL|PASS"
//
//	# 运行单个 BC 测试
//	MANUAL=1 go test -v -count=1 ./internal/manualtest/iam/...
//	MANUAL=1 go test -v -count=1 ./internal/manualtest/app/...
//
//	# 串行执行（服务端压力大时）
//	MANUAL=1 go test -v -count=1 -p 1 ./internal/manualtest/...
//
//	# 运行单个测试函数

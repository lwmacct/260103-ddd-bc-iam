...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestOrgCRUD ./internal/integration/org/
func TestOrgCRUD(t *testing.T) {
	...
	// 验证列表中包含创建的组织
	orgIDs := manualtest.ExtractIDs(orgs, func(o org.OrgDTO) uint { return o.ID })
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestOrgListPagination ./internal/integration/org/
func TestOrgListPagination(t *testing.T) {
	...
	if meta1.TotalPages > 1 {
		...
		// 验证无重复 ID
		ids1 := manualtest.ExtractIDs(page1, func(o org.OrgDTO) uint { return o.ID })
		ids2 := manualtest.ExtractIDs(page2, func(o org.OrgDTO) uint { return o.ID })
		for _, id := range ids1 {
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestOrgStatusUpdate ./internal/integration/org/
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestOrgMemberCRUD ./internal/integration/org/
func TestOrgMemberCRUD(t *testing.T) {
	...
	// 步骤 2: 创建测试用户
	t.Log("\n步骤 2: 创建测试用户")
	testUser := manualtest.CreateTestUser(t, c, "orgmember")
	...
	member, err := manualtest.Post[org.MemberDTO](c, memberPath, addMemberReq)
	...
	// 步骤 4: 获取成员列表
	t.Log("\n步骤 4: 获取成员列表")
	members, meta, err := manualtest.GetList[org.MemberDTO](c, memberPath, map[string]string{
		"page":  "1",
		"limit": "10",
	})
	...
	_, err = manualtest.Put[any](c, updatePath, updateRoleReq)
	require.NoError(t, err, "更新成员角色失败")
	t.Logf("  角色更新成功!")

	// TODO: 成员详情端点不存在，需要添加或通过列表验证
	// updatedMember, err := manualtest.Get[org.MemberDTO](c, fmt.Sprintf("%s/%d", memberPath, testUser.ID), nil)
	...
	// 验证成员已移除
	membersAfter, _, err := manualtest.GetList[org.MemberDTO](c, memberPath, nil)
	require.NoError(t, err, "获取成员列表失败")
	userIDsAfter := manualtest.ExtractIDs(membersAfter, func(m org.MemberDTO) uint { return m.UserID })
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestOrgMemberLastOwnerProtection ./internal/integration/org/
func TestOrgMemberLastOwnerProtection(t *testing.T) {
	...
	t.Log("\n步骤 2: 获取成员列表")
	members, _, err := manualtest.GetList[org.MemberDTO](c, fmt.Sprintf("/api/org/%d/members", orgID), nil)
	require.NoError(t, err, "获取成员列表失败")
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestUserOrgs ./internal/integration/org/
func TestUserOrgs(t *testing.T) {
	c := manualtest.LoginAsAdmin(t)

	t.Log("\n获取用户加入的组织列表...")
	userOrgs, _, err := manualtest.GetList[org.UserOrgDTO](c, "/api/user/orgs", nil)
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestOrgWithInvalidData ./internal/integration/org/

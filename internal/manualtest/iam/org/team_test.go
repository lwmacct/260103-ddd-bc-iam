...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestTeamCRUD ./internal/integration/org/
func TestTeamCRUD(t *testing.T) {
	...
	// 测试 2: 获取团队列表
	t.Log("\n测试 2: 获取团队列表")
	teams, meta, err := manualtest.GetList[org.TeamDTO](c, teamBasePath(testOrgID), map[string]string{
		"page":  "1",
		"limit": "100", // 足够大的 limit 以包含新创建的团队
	})
	...
	// 验证列表中包含创建的团队
	teamIDs := manualtest.ExtractIDs(teams, func(tm org.TeamDTO) uint { return tm.ID })
	assert.Contains(t, teamIDs, teamID, "列表中应包含新创建的团队")

	// 测试 3: 获取团队详情
	t.Log("\n测试 3: 获取团队详情")
	teamDetail, err := manualtest.Get[org.TeamDTO](c, teamPath(testOrgID, teamID), nil)
	...
	updatedTeam, err := manualtest.Put[org.TeamDTO](c, teamPath(testOrgID, teamID), updateReq)
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestTeamListByOrg ./internal/integration/org/
func TestTeamListByOrg(t *testing.T) {
	...
	team1, err := manualtest.Post[org.TeamDTO](c, teamBasePath(testOrgID), team1Req)
	...
	team2, err := manualtest.Post[org.TeamDTO](c, teamBasePath(testOrgID), team2Req)
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestTeamOrgIsolation ./internal/integration/org/
func TestTeamOrgIsolation(t *testing.T) {
	...
	_, err = manualtest.Get[org.TeamDTO](c, teamPath(invalidOrgID, teamID), nil)
	...
	_, err = manualtest.Put[org.TeamDTO](c, teamPath(invalidOrgID, teamID), updateReq)
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestTeamMemberCRUD ./internal/integration/org/
func TestTeamMemberCRUD(t *testing.T) {
	...
	// 步骤 2: 创建测试用户并加入组织
	t.Log("\n步骤 2: 创建测试用户并加入组织")
	testUser := manualtest.CreateTestUser(t, c, "teammember")
	...
	_, err = manualtest.Post[org.MemberDTO](c, fmt.Sprintf("/api/org/%d/members", testOrgID), addOrgMemberReq)
	...
	teamMember, err := manualtest.Post[org.TeamMemberDTO](c, teamMembersBasePath(testOrgID, teamID), addTeamMemberReq)
	...
	// 步骤 4: 获取团队成员列表
	t.Log("\n步骤 4: 获取团队成员列表")
	members, meta, err := manualtest.GetList[org.TeamMemberDTO](c, teamMembersBasePath(testOrgID, teamID), map[string]string{
		"page":  "1",
		"limit": "10",
	})
	...
	// 验证列表中包含新成员
	userIDs := manualtest.ExtractIDs(members, func(m org.TeamMemberDTO) uint { return m.UserID })
	...
	// 验证成员已移除
	membersAfter, _, err := manualtest.GetList[org.TeamMemberDTO](c, teamMembersBasePath(testOrgID, teamID), nil)
	require.NoError(t, err, "获取团队成员列表失败")
	userIDsAfter := manualtest.ExtractIDs(membersAfter, func(m org.TeamMemberDTO) uint { return m.UserID })
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestTeamMemberRequiresOrgMembership ./internal/integration/org/
func TestTeamMemberRequiresOrgMembership(t *testing.T) {
	...
	// 步骤 2: 创建测试用户（但不加入组织）
	t.Log("\n步骤 2: 创建测试用户")
	testUser := manualtest.CreateTestUser(t, c, "nonorgmember")
	t.Logf("  用户创建成功! ID: %d", testUser.ID)
	...
	_, err = manualtest.Post[org.TeamMemberDTO](c, teamMembersBasePath(testOrgID, teamID), addTeamMemberReq)
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestUserTeams ./internal/integration/org/
func TestUserTeams(t *testing.T) {
	c := manualtest.LoginAsAdmin(t)

	t.Log("\n获取用户加入的团队列表...")
	userTeams, _, err := manualtest.GetList[org.UserTeamDTO](c, "/api/user/teams", nil)
...
// 手动运行:
//
//	MANUAL=1 go test -v -run TestTeamWithInvalidOrgID ./internal/integration/org/
